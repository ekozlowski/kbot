...
def get_darksky_data(latitude=LATITUDE, longitude=LONGITUDE):
    if DEBUG:
        if os.path.exists(
            "./darksky_data.json"
        ):  # remove "False" to debug and NOT pull from live.
            print("Pulling from DISK")
            data = open("./darksky_data.json", "r").read()
    else:
        print("Pulling from LIVE")
        url = (
            f"https://api.darksky.net/forecast/{DARKSKY_API_KEY}/{latitude},{longitude}"
        )
        print(url)
...
def get_week_forecast():
    ...
    for i, d in enumerate(daily_data):
        if i == 0:
            ...
            day = datetime.datetime.fromtimestamp(d.get("time")).strftime(
                "%A - (%m/%d):"
            )
        forecast += f"{day} {icon_map.get(d.get('icon')) or d.get('icon')} {d.get('summary')} High: {d.get('temperatureMax'):.0f}F Low: {d.get('temperatureMin'):.0f}F Wind: {d.get('windSpeed'):.0f}mph from the {get_wind_direction(d.get('windBearing'))}"
        forecast += "\n"
