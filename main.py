...
from handlers import (
    weather,
)  # Have to import weather *after* the overrides, so it has the API key for DARKSKY.  :(
...
def message_callback(channel, as_user):
    def send(message):
        slack_client.api_call(
            "chat.postMessage", channel=channel, text=message, as_user=as_user
        )
...
def handle_command(command, channel):
    ...
    if cmd not in handlers:
        ...
        response = handlers.get(cmd).handle(
            command, message_callback(channel, as_user=True)
        )
        ...
        while True:
            ...
            if command and channel:
                try:
                    ...
                    raise
                    log.error(
                        f"Problem handling command: {command} in channel {channel}"
                    )
